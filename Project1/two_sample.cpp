...
vector<double> mean_diff_sampling_distribution(
  ...
  for(int i = 0; i<1000; i++){
    double mean_a = mean(bootstrap_resample(data_A, i));
    double mean_b = mean(bootstrap_resample(data_B, i));
    means.push_back(mean_a - mean_b);
  }
  return means;
...
pair<double, double> confidence_interval(vector<double> v, double width) {
  // TODO: Implement this function, removing the assert(false); placeholder.
  double upper = 0.5 + width/2.0;
  double lower = 0.5 - width/2.0;
