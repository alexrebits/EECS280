...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node * find_impl(Node *node, const T &query, Compare less) {
    ...
    return find_impl(next,query,less);
  }
    //   if(less(node->datum,query)){
    //   if(node->right){
    //     next = node->right;
  ...
  static Node * insert_impl(Node *node, const T &item, Compare less) { 
    if(empty_impl(node)){ //base case: empty spot
      node = new Node;
      node->datum = item;
      node->left = nullptr;
      node->right = nullptr;
    }
    //if next == nullptr, then we reach base case
    else if(less(node->datum,item)){node->right= insert_impl(node->right,item,less);}
    else if(less(item,node->datum)){node->left = insert_impl(node->left,item,less);} 
    return node; //this means the item has been inserted and we can return
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    ...
    if((node->left)&&
    (less(node->datum,max_element_impl(node->left)->datum))){return false;}
    else if((node->right)&&
    (less(min_element_impl(node->right)->datum, node->datum))){return false;}
    else {
    ...
    //     if(less(node->datum,(node->left)->datum)){return false;}
    // else if(less((node->right)->datum, node->datum)){return false;}
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) { 
    ...
    if(empty_impl(node)){return nullptr;} //empty
    else if(less(val,node->datum)){ // contemplate going into left subtree
      if(empty_impl(node->left)){return node;}
      else if(less(val,(max_element_impl(node->left))->datum)){ 
        return min_greater_than_impl(node->left,val,less);
      }
       // the root node is the min greater node
        return node;
      
    }
    else{ //going into right subtree, either we find it or we don't
