...
private:
  ...
  // A custom comparator
  class PairComp { //compare only the .first "key_type" value???
    private:
  Key_compare k;
  
   public:

    bool operator()(const Pair_type &first_in, const Pair_type &second_in) const{
      return k(first_in.first,second_in.first); 
    }
...
//   // temp.second =Value_type(); ?????? how are you supposed to use Value_type???
//   return tree.find({k,Value_type()});
//   //return the non null pointer if it exists, return nullptr if it doesn't
// }
...
//   if((*(find(k))).first){
//     return (*(find(k))).second; //maybe not right syntax?
//   }
//   else{
//     return insert(k)->first; // issue!!!
//   }
// }
